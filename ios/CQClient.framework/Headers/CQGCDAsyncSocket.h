...
@class CQGCDAsyncReadPacket;
@class CQGCDAsyncWritePacket;
@class CQGCDAsyncSocketPreBuffer;
@protocol CQGCDAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const CQGCDAsyncSocketException;
extern NSString *const CQGCDAsyncSocketErrorDomain;

extern NSString *const CQGCDAsyncSocketQueueName;
extern NSString *const CQGCDAsyncSocketThreadName;

extern NSString *const CQGCDAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const CQGCDAsyncSocketUseCFStreamForTLS;
#endif
#define CQGCDAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define CQGCDAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define CQGCDAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const CQGCDAsyncSocketSSLPeerID;
extern NSString *const CQGCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const CQGCDAsyncSocketSSLProtocolVersionMax;
extern NSString *const CQGCDAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const CQGCDAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const CQGCDAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const CQGCDAsyncSocketSSLDiffieHellmanParameters;
#endif

#define CQGCDAsyncSocketLoggingContext 65535


typedef NS_ERROR_ENUM(CQGCDAsyncSocketErrorDomain, CQGCDAsyncSocketError) {
	CQGCDAsyncSocketNoError = 0,           // Never used
	CQGCDAsyncSocketBadConfigError,        // Invalid configuration
	CQGCDAsyncSocketBadParamError,         // Invalid parameter was passed
	CQGCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	CQGCDAsyncSocketReadTimeoutError,      // A read operation timed out
	CQGCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	CQGCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	CQGCDAsyncSocketClosedError,           // The remote peer closed the connection
	CQGCDAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface CQGCDAsyncSocket : NSObject
...
- (instancetype)initWithDelegate:(nullable id<CQGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<CQGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<CQGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error;

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<CQGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<CQGCDAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<CQGCDAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<CQGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<CQGCDAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<CQGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
@protocol CQGCDAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(CQGCDAsyncSocket *)sock;
...
**/
- (void)socket:(CQGCDAsyncSocket *)sock didAcceptNewSocket:(CQGCDAsyncSocket *)newSocket;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)socket:(CQGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(CQGCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(CQGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(CQGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(CQGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(CQGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(CQGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(CQGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(CQGCDAsyncSocket *)sock;
...
**/
- (void)socketDidDisconnect:(CQGCDAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socketDidSecure:(CQGCDAsyncSocket *)sock;
...
**/
- (void)socket:(CQGCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
